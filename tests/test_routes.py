...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        ...
        self.assertEqual(result["id"], test_product.id)
        self.assertEqual(result["name"], test_product.name)
        self.assertEqual(result["description"], test_product.description)
        self.assertEqual(Decimal(result["price"]), test_product.price)
        self.assertEqual(result["available"], test_product.available)
        self.assertEqual(result["category"], test_product.category.name)
    ...
    def test_update_product(self):
        ...
        new_product = response.get_json()
        new_product["description"] = "unkown"
        response = self.client.put(BASE_URL + f'/{new_product["id"]}', json=new_product)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        updated_product = response.get_json()
        self.assertEqual(updated_product["description"], "unkown")

    def test_update_product_not_found(self):
        """It should not Update a Product thats not found"""
        test_product = ProductFactory()
        response = self.client.put(BASE_URL + "/0", json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND) 
    ...
    def test_deleteproduct_not_found(self):
        """It should not Update a Product thats not found"""
        test_product = ProductFactory()
        response = self.client.delete(BASE_URL + "/0", json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND) 
    ...
    def test_query_by_availabilty(self):
